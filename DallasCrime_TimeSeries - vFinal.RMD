...
crime = read.csv("G:/My Drive/Masters/Spring 2021 (ML 1 + TS)/DS 6373 - Time Series/Project/DallasCrime_Agg.csv")
head(crime, 5)
NROW(crime) 
```
...
### -------------------------------- Modeling - Multivariate - Vector Autoregressive (VAR) --------------------------------
__Model 4 - Multivariate Assumption Checking__
```{r,warning=FALSE,message=FALSE}
NROW(crime_trimmed)
ggpairs(crime_trimmed[2:6]) #matrix of scatter plots

#forecasting feature variables
#checking assumptions of all exogenous variables
plotts.sample.wge(crime_trimmed$Victim_Female)
plotts.sample.wge(crime_trimmed$Victim_Male)
plotts.sample.wge(crime_trimmed$Victim_Age)
plotts.sample.wge(crime_trimmed$Victim_Race)

# #forecasting Victim_Female
# aic5.wge(crime_trimmed$Victim_Female,p=0:10, q=0:5) #p=5 ,q=4
# est_female = est.arma.wge(crime_trimmed$Victim_Female, p=5, q=4)
# female_short_forecast = fore.arma.wge(crime_trimmed$Victim_Female,phi=est_female$phi, theta=est_female$theta,n.ahead = 60)
# female_long_forecast = fore.arma.wge(crime_trimmed$Victim_Female,phi=est_female$phi, theta=est_female$theta,n.ahead = 1095)
# 
# #forecasting Victim_Age
# aic5.wge(crime_trimmed$Victim_Age,p=0:10, q=0:5) #p=4 ,q=3
# est_age = est.arma.wge(crime_trimmed$Victim_Age, p=4, q=3)
# age_short_forecast = fore.arma.wge(crime_trimmed$Victim_age,phi=est_age$phi, theta=est_age$theta,n.ahead = 60)
# age_long_forecast = fore.arma.wge(crime_trimmed$Victim_age,phi=est_age$phi, theta=est_age$theta,n.ahead = 1095)
# 
# 
# #forecasting Victim_Race
# aic5.wge(crime_trimmed$Victim_Race,p=0:10, q=0:5) #p=5 ,q=1
# est_race = est.arma.wge(crime_trimmed$Victim_Race, p=5, q=1)
# race_short_forecast = fore.arma.wge(crime_trimmed$Victim_race,phi=est_race$phi, theta=est_race$theta,n.ahead = 60)
# race_long_forecast = fore.arma.wge(crime_trimmed$Victim_race,phi=est_race$phi, theta=est_race$theta,n.ahead = 1095)

crime = crime_trimmed$Tot_Crime_Count
female = crime_trimmed$Victim_Female
male = crime_trimmed$Victim_Male
age = crime_trimmed$Victim_Age
race = crime_trimmed$Victim_Race

#differencing to make exogenous variables stationary
crime_d1 = artrans.wge(crime_trimmed$Tot_Crime_Count,1)
female_d1 = artrans.wge(crime_trimmed$Victim_Female,1)
male_d1 = artrans.wge(crime_trimmed$Victim_Male,1)
age_d1 = artrans.wge(crime_trimmed$Victim_Age,1)
race_d1 = artrans.wge(crime_trimmed$Victim_Race,1)

#checking differenced variables for stationarity
plotts.sample.wge(crime_d1, arlimits=TRUE)
plotts.sample.wge(female_d1, arlimits=TRUE)
plotts.sample.wge(male_d1, arlimits=TRUE)
plotts.sample.wge(age_d1, arlimits=TRUE)
plotts.sample.wge(race_d1, arlimits=TRUE)
```

__Model 4 - Vector Autoregressive (VAR) - Forecasting Short Horizon (60 days)__
```{r,warning=FALSE,message=FALSE}
#adding time variable
?mlp
time_diff = data.frame(time = seq(1,2487,1))
time = data.frame(time = seq(1,2488,1))

#running VARSelect on differenced data
VARselect(cbind(crime_d1, female_d1,age_d1,race_d1),lag.max = 10, type = "const") #7
#running VARSelect on nonstationary data
VARselect(cbind(crime, female,age,race),lag.max = 10, type = "const") #7

#nonstationary VAR
crimeVAR = VAR(cbind(crime, female,age, race), type="both",p=7)
AIC(crimeVAR) #56292.65
#nonstationary VAR with time
crimeVAR_time = VAR(cbind(crime, female,age, race), type="both",p=7,exogen=time)
AIC(crimeVAR_time) #56300.65
#statioanry VAR model
crimediffVAR = VAR(cbind(crime_d1, female_d1,age_d1,race_d1), type="both",p=7)
AIC(crimediffVAR) #56991
#stationary VAR model with time
crimediffVAR_time = VAR(cbind(crime_d1, female_d1,age_d1,race_d1), type="both",p=7,exogen=time_diff)
AIC(crimediffVAR_time) #56999

#forecasting short horizon nostationary VAR since it is VAR model with lowest AIC
VAR_preds_short = predict(crimeVAR,n.ahead =60)
head(VAR_preds_short$fcst$crime,5)

# nonstationary VAR short horizon forecast
plot(seq(1,2488,1), crime, type = "l",xlim = c(1,2548), ylab = "Total Crime", main = "VAR Model - 60 Day Forecast")
lines(seq(2489,2548,1), VAR_preds_short$fcst$crime[,1], type = "l", col = "red")

# nonstationary ASE
m4_VAR_short_ASE = mean((crime[(length(crime)-60+1):length(crime)] - VAR_preds_short$fcst$crime[,1])^2)
m4_VAR_short_ASE #1144.896

```

__Model 4 - Vector Autoregressive (VAR) - Forecasting Long Horizon (3 Years)__
```{r,warning=FALSE,message=FALSE}
VAR_preds_long = predict(crimeVAR,n.ahead =1095)
head(VAR_preds_long$fcst$crime,5)

# nonstationary VAR long horizon forecast
plot(seq(1,2488,1), crime, type = "l",xlim = c(1,3583), ylab = "Total Crime", main = "VAR Model - 3 Year Forecast")
lines(seq(2489,3583,1), VAR_preds_long$fcst$crime[,1], type = "l", col = "red")

# nonstationary ASE
m4_VAR_long_ASE = mean((crime[(length(crime)-1095+1):length(crime)] - VAR_preds_long$fcst$crime[,1])^2)
m4_VAR_long_ASE #691.8346
```

### -------------------------------- Modeling - Neural Network Models --------------------------------
__Model 5 - Neural Network (NN) - Univariate__
We will next model a Neural Network (NN) model using rep of 20. A thing to note here is that the MLP model is highly inconsistent and and computationally taxing. Therefore, only 
...
#subsetting data
crime_small_short = ts(crime_trimmed$Tot_Crime_Count[1:2428])
crime_small_long = ts(crime_trimmed$Tot_Crime_Count[1:1393])

# Univariate MLP Model
# create short horizon MLP model 
fit_mlp_crime_short = mlp(crime_small_short)
fit_mlp_crime_short
plot(fit_mlp_crime_short)
# create long horizon MLP model 
fit_mlp_crime_long = mlp(crime_small_long,rep=5)
fit_mlp_crime_long
plot(fit_mlp_crime_long)

#forecast short horizon model
fore_mlp_crime_short = forecast(fit_mlp_crime_short, h = 60)
plot(fore_mlp_crime_short)
#forecast long horizon model
fore_mlp_crime_long = forecast(fit_mlp_crime_long, h = 1095)
plot(fore_mlp_crime_long)

#short horizon ASE
m5_MLP_short_ASE = mean((crime[(length(crime)-60+1):length(crime)]-fore_mlp_crime_short$mean)^2)
m5_MLP_short_ASE #875.0553

#long horizon ASE
m5_MLP_long_ASE = mean((crime[(length(crime)-1095+1):length(crime)]-fore_mlp_crime_long$mean)^2)
m5_MLP_long_ASE #950.2433
```

### -------------------------------- Modeling - Ensemble Models --------------------------------
__Model 6 - Ensemble Model__
```{r,warning=FALSE,message=FALSE}
#creating ensemble model using best performing model from each of the prior 5 models
ensemble_short  = (m1_ARMA55_forecast_short$f + sigplusnoise_forecast_short$f + VAR_preds_short$fcst$crime[,1] + fore_mlp_crime_short$mean)/4
ensemble_long  = (m1_ARMA55_forecast_long$f + sigplusnoise_forecast_long$f + VAR_preds_long$fcst$crime[,1] + fore_mlp_crime_long$mean)/4

#plot short term horizon
plot(seq(1,2488,1), crime, type = "l",xlim = c(1,2548), ylab = "Total Crime", main = "Ensemble Model - 60 Day Forecast")
lines(seq(2489,2548,1), ensemble_short, type = "l", col = "red")

#plot long term horizon
plot(seq(1,2488,1), crime, type = "l",xlim = c(1,3583), ylab = "Total Crime", main = "Ensemble Model - 3 Year Forecast")
lines(seq(2489,3583,1), ensemble_long, type = "l", col = "red")

# short term ASE
m6_Ensemble_short_ASE = mean((crime[(length(crime)-60+1):length(crime)] - ensemble_short)^2)
m6_Ensemble_short_ASE #964.7032

# short term ASE
m6_Ensemble_long_ASE = mean((crime[(length(crime)-1095+1):length(crime)] - ensemble_long)^2)
m6_Ensemble_long_ASE #595.3352
```
